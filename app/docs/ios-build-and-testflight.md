...
> 다른 언어 앱 리포에 그대로 옮겨 붙이는 안내서. 실제 동작 중인 구현: `talkverselab/talkverse-th`
...
| 번들 ID | `ios/Runner.xcodeproj/project.pbxproj` 의 `PRODUCT_BUNDLE_IDENTIFIER` (예: `com.talkverse.mongolianUniverse`). 앱마다 고유. |
...
`CFBundleDisplayName` 이 홈 화면 이름이다(예: 몽골어유니버스).
...
`ios/Podfile` 이 없어도 `flutter build ios` 가 만들어 준다. 이 앱은 이 단계가 첫 시도에 성공했다(6분).
...
python -X utf8 tools/ios/make_ios_cert.py --key-id XXXXXXXXXX --issuer-id <UUID> --p8 AuthKey_XXXXXXXXXX.p8 \
    --bundle-id com.talkverse.mongolianUniverse --app-name "몽골어유니버스" --out ./ios_secrets
```
